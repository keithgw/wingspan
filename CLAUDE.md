...
- **Training**: `LinearPolicy` (numpy-based, interpretable weights) trained via REINFORCE self-play. `featurizer.py` converts GameState to 22 named features (including strategic calculations and deck composition awareness). `self_play.py` runs games and collects experience. `train.py` is the CLI entry point
